...
describe('wiki creation', () => {
  ...
  it('lists all active wikis ordered by slug', async () => {
    const { kit } = await createTestKit()
    should(await kit.listWikis()).deepEqual([])
    await kit.createWiki({ slug: 'zebra', content: 'z', actor: human })
    await kit.createWiki({ slug: 'acme', content: 'a', title: 'Acme', actor: human })
    const wikis = await kit.listWikis()
    wikis.map((wiki) => wiki.slug).should.deepEqual(['acme', 'zebra'])
    wikis[0].title.should.equal('Acme')
    wikis.every((wiki) => wiki.id === wiki.wikiId).should.be.true()
  })

  it('creates a wiki with a nested target in one commit', async () => {
    const { kit, db } = await createTestKit()
    const leaf = await kit.createWiki({
      slug: 'acme', path: 'about.foo', content: 'Foo docs', title: 'Foo', actor: human
    })
    leaf.path.should.equal('about.foo')
    leaf.content.should.equal('Foo docs')
    leaf.title.should.equal('Foo')

    const root = await kit.resolveWikiBySlug({ slug: 'acme' })
    root.content.should.equal('')
    should(root.title).be.null()
    const about = await kit.getNode({ wikiId: root.id, path: 'about' })
    about.content.should.equal('')

    // root, intermediate, and leaf all belong to a single commit
    commitCount(db, root.id).should.equal(1)
    root.commitId.should.equal(leaf.commitId)
    about.commitId.should.equal(leaf.commitId)
    const commit = await kit.getCommit({ wikiId: root.id, commitId: leaf.commitId })
