...
const config = {
  url: localStorage.getItem('waterfront.url') || 'http://localhost:3000',
  token: localStorage.getItem('waterfront.token') || 'dev-token',
  wiki: localStorage.getItem('waterfront.wiki') || 'acme'
}
...
elements.settingsForm.addEventListener('submit', (event) => {
  ...
  localStorage.setItem('waterfront.url', config.url)
  localStorage.setItem('waterfront.token', config.token)
  localStorage.setItem('waterfront.wiki', config.wiki)
