/**
 * wiki-kit — the waterfront domain core.
 *
...
export function createWikiKit ({ db }) {
  ...
  function buildTree (nodes, rootPath, depth) {
    ...
    for (const children of childrenByParent.values()) {
      children.sort((a, b) => (a.slug < b.slug ? -1 : a.slug > b.slug ? 1 : 0))
    }
  ...
  const setNodeTx = db.transaction((args) => {
    ...
    // Ensure intermediate parents exist, creating empty nodes in this commit.
    ...
    ftsUpsert(id)
    return { node: rowToNode(getActiveNodeByPath(wikiId, path)), changed: true, created: true }
  })

  const createWikiTx = db.transaction((args) => {
    const { slug, title, content, metadata, actor, message } = args
    ...
    ftsUpsert(id)
    return rowToNode(getActiveRoot(id))
  })
  ...
  return {
    ...
    async createWiki ({ slug, title, content, metadata, actor, message }) {
      assertValidSlug(slug)
      assertTitle(title)
      assertContent(content)
      assertMetadata(metadata)
      assertActor(actor)
      return createWikiTx.immediate({ slug, title, content, metadata, actor, message })
    },
