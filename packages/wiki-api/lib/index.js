/**
 * wiki-api — thin HTTP layer over wiki-kit.
 *
 * Owns authentication, authorization, transport validation, and
 * error mapping. Contains no SQL and no domain rules: full public
 * paths are resolved to (wikiId, relative path) and handed to the kit.
 */
 ...
 * @param {object} [options.fastifyOptions] passed to Fastify()
 */
export function createWikiApi ({ kit, auth, fastifyOptions = {} }) {
  ...
  const app = Fastify({ bodyLimit: 4 * 1024 * 1024, ...fastifyOptions })

  // Permissive CORS so wiki-web (or any browser client) can call the
  ...
  app.options('/rpc', async (request, reply) => reply.code(204).send())
  ...
  const methods = {
    async 'wiki.get' (principal, params) {
      ...
      await auth.authorize(principal, { action: 'wiki:read', wiki: rootSlug, path: fullPath })
    ...
    async 'wiki.set' (principal, params) {
      ...
      if (!wiki && relPath === '') {
        await auth.authorize(principal, { action: 'wiki:create', wiki: slug, path: fullPath })
      ...
      await auth.authorize(principal, { action: 'wiki:write', wiki: slug, path: fullPath })
    ...
    async 'wiki.move' (principal, params) {
      ...
      await auth.authorize(principal, { action: 'wiki:write', wiki: rootSlug, path: from })
    ...
    async 'wiki.remove' (principal, params) {
      const fullPath = requireString(params, 'path')
      const { wiki, relPath, rootSlug } = await resolveWiki(fullPath)
      await auth.authorize(principal, { action: 'wiki:delete', wiki: rootSlug, path: fullPath })
    ...
    async 'wiki.getCommit' (principal, params) {
      ...
      await auth.authorize(principal, { action: 'wiki:read', wiki: rootSlug, path: wikiSlug })
  ...
  app.post('/rpc', async (request, reply) => {
    ...
    if (!body || typeof body !== 'object' || Array.isArray(body) ||
      ...
      return reply.send(rpcError(body?.id, RPC.INVALID_REQUEST, 'invalid JSON-RPC 2.0 request'))
    }
    const { id, method } = body
    const params = body.params ?? {}
    if (typeof params !== 'object' || Array.isArray(params)) {
      return reply.send(rpcError(id, RPC.INVALID_PARAMS, 'params must be an object'))
    }
    const handler = methods[method]
    if (!handler) {
      return reply.send(rpcError(id, RPC.METHOD_NOT_FOUND, `method not found: ${method}`))
    }
    try {
      ...
      return reply.send({ jsonrpc: '2.0', id: id ?? null, result })
    } catch (error) {
      if (!(error instanceof WikiError)) request.log.error(error)
      return reply.send(mapError(id, error))
    }
  })

  app.setErrorHandler((error, request, reply) => {
    // Body parse failures and other transport-level errors.
    if (error.statusCode === 400) {
      return reply.code(200).send(rpcError(null, RPC.PARSE_ERROR, 'parse error'))
    }
    request.log.error(error)
    return reply.code(200).send(rpcError(null, RPC.INTERNAL_ERROR, 'internal error'))
  })
