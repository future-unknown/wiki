...
import { UnauthenticatedError, UnauthorizedError } from './auth.js'

export { createStaticTokenAuth, UnauthenticatedError, UnauthorizedError } from './auth.js'
export { METHOD_DECLARATIONS, validateParams } from './declarations.js'
import { METHOD_DECLARATIONS, validateParams } from './declarations.js'
export { openDatabase } from './db.js'
export { openRecordStore } from './records.js'
...
// A root node's slug is its wiki's address, which is the caller's to
// name (see `names`); every other node's slug is its own.
function decorateNode (rootSlug, node) {
  const addressed = node.path === '' ? { slug: rootSlug } : {}
  return { ...node, ...addressed, fullPath: fullPathOf(rootSlug, node.path) }
}

// Callers address a wiki by the slug it is stored under.
const STORED_NAMES = Object.freeze({
  stored: (slug) => slug,
  addressed: (slug) => slug
})
 ...
 *
 * `names` separates the slug a caller addresses a wiki by from the slug
 * the kit stores it under, for a host whose addresses carry a name that
 * can change (an owner's, say) while storage should not have to:
 * `stored(addressed)` is the stored slug, or null for no wiki this
 * caller can address; `addressed(stored)` is the caller's slug for a
 * stored wiki, or null to leave it out of listings. Every path in and
 * out uses the addressed slug. Without it the two are the same.
 *
 * @param {{ kit: object, auth?: { authorize?: Function }, names?: { stored: Function, addressed: Function } }} options
 */
export function createWikiMethods ({ kit, auth, names = STORED_NAMES }) {
  ...
  async function resolveWiki (fullPath) {
    ...
    const wiki = await storedWiki(slug)
    if (!wiki) throw new NotFoundError('wiki not found', { wiki: slug })
    return { wiki, relPath: path, rootSlug: slug }
  }

  async function storedWiki (slug) {
    const stored = names.stored(slug)
    return stored ? kit.resolveWikiBySlug({ slug: stored }) : null
  }
  ...
  const table = {
    async 'wiki.list' (principal) {
      ...
      // A listing shows what the principal may read: each wiki is
      // authorized individually, and refused ones are simply absent.
      const visible = []
      for (const wiki of wikis) {
        const slug = names.addressed(wiki.slug)
        if (!slug) continue
        try {
          await authorize(principal, { action: 'wiki:read', wiki: slug, path: slug })
          visible.push(decorateNode(slug, wiki))
        } catch (error) {
    ...
    async 'wiki.set' (principal, params) {
      ...
      if (!wiki) {
        ...
        const stored = names.stored(slug)
        if (!stored) throw new NotFoundError('wiki not found', { wiki: slug })
    ...
    async 'wiki.log' (principal, params) {
      const fullPath = params.path
      const { wiki, relPath, rootSlug } = await resolveWiki(fullPath)
      await authorize(principal, { action: 'wiki:read', wiki: rootSlug, path: fullPath })
      const entries = await kit.getLog({
        wikiId: wiki.id, path: relPath, limit: params.limit, before: params.before
      })
      // Each change names its own page, so each carries its full path.
      return entries.map((entry) => ({
        ...entry,
        changes: entry.changes.map((change) => ({ ...change, fullPath: fullPathOf(rootSlug, change.path) }))
      }))
    },
    ...
    async 'wiki.put' (principal, params) {
      const fullPath = params.path
      const { wiki, relPath, rootSlug } = await resolveWiki(fullPath)
      // Writing records is its own action, distinct from authoring:
      // emitter principals can be scoped to report data and nothing else.
      await authorize(principal, { action: 'wiki:put', wiki: rootSlug, path: fullPath })
      const record = await kit.putRecord({
        wikiId: wiki.id,
        path: relPath,
        value: params.value,
        ts: params.ts,
        ifVersion: params.ifVersion,
        actor: principal.actor
      })
      return { fullPath, record }
    },

    async 'wiki.del' (principal, params) {
      const fullPath = params.path
      const key = params.key
      const { wiki, relPath, rootSlug } = await resolveWiki(fullPath)
      // Deleting a record is curation, not emission: it needs the
      // write grant, so a put-scoped emitter cannot destroy data.
      await authorize(principal, { action: 'wiki:write', wiki: rootSlug, path: fullPath })
      const result = await kit.deleteRecord({
        wikiId: wiki.id, path: relPath, key, actor: principal.actor
      })
      return { fullPath, record: result.record }
    },
    ...
    async 'wiki.data' (principal, params) {
      ...
      const { wiki, relPath, rootSlug } = await resolveWiki(fullPath)
      await authorize(principal, { action: 'wiki:read', wiki: rootSlug, path: fullPath })
      const result = await kit.getRecords({
        wikiId: wiki.id,
        path: relPath,
        key: params.key,
    ...
    async 'wiki.meta' (principal, params) {
      const fullPath = params.path
      const { wiki, relPath, rootSlug } = await resolveWiki(fullPath)
      await authorize(principal, { action: 'wiki:write', wiki: rootSlug, path: fullPath })
      const result = await kit.mergeMetadata({
        wikiId: wiki.id,
        path: relPath,
        metadata: params.metadata,
      ...
      return { node: decorateNode(rootSlug, result.node), changed: result.changed }
    },

    async 'wiki.getCommit' (principal, params) {
      const wikiSlug = params.wiki
    ...
    async 'wiki.revision' (principal, params) {
      const wikiSlug = params.wiki
      const revisionId = params.revisionId
      const { wiki, rootSlug } = await resolveWiki(wikiSlug)
      // A revision is page content, so it is read at the page: the
      // node's current path is what the guard sees, as for wiki.get.
      const revision = await kit.getRevision({ wikiId: wiki.id, revisionId })
      const fullPath = fullPathOf(rootSlug, revision.path)
      await authorize(principal, { action: 'wiki:read', wiki: rootSlug, path: fullPath })
      return decorateNode(rootSlug, revision)
    },
  ...
  // Every call is checked against its declaration first, so a handler
  // reads params already known to be shaped as declared.
  const methods = {}
  for (const [name, handler] of Object.entries(table)) {
    if (!METHOD_DECLARATIONS[name]) throw new Error(`${name} has no declaration`)
    methods[name] = async (principal, params = {}) => {
      validateParams(name, params)
      return handler(principal, params)
    }
  }
  for (const name of Object.keys(METHOD_DECLARATIONS)) {
    if (!methods[name]) throw new Error(`${name} is declared but not implemented`)
  }
  return methods
}
