...
export function createWikiMethods ({ kit, auth }) {
  ...
  return {
    ...
    async 'wiki.dataSummary' (principal, params) {
      const fullPath = requireString(params, 'path')
      const { wiki, relPath, rootSlug } = await resolveWiki(fullPath)
      await authorize(principal, { action: 'wiki:read', wiki: rootSlug, path: fullPath })
      const summary = await kit.getDataSummary({ wikiId: wiki.id, path: relPath })
      // Each entry names its own page: summaries span a subtree.
      return summary.map((entry) => ({ ...entry, fullPath: fullPathOf(rootSlug, entry.path) }))
    },
