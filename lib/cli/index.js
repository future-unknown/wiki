...
import { renderTree, renderSearchResults, renderHistory, renderData, renderDataSummary } from './render.js'
...
export async function main (argv) {
  ...
  try {
    switch (commandName) {
      ...
      case 'push': {
        const [path, inline] = positionals
        if (!path || positionals.length > 2) throw new UsageError('usage: wiki push <path> [json]')
        ...
        if (raw === undefined || raw.trim() === '') {
          throw new UsageError('no payload supplied (inline argument or stdin)')
        }
        let payload
        try {
        ...
        const datum = await client.push(path, payload, { ts: values.ts })
        if (json) out(JSON.stringify(datum, null, 2))
        else err(`pushed to ${datum.fullPath} (${datum.ts})`)
        break
      }

      case 'data': {
        const [path] = positionals
        if (!path || positionals.length !== 1) throw new UsageError('usage: wiki data <path>')
        if (values.summary) {
          if (values.latest || values.since || values.until || values.limit) {
            throw new UsageError('--summary stands alone — drop --latest/--since/--until/--limit')
          }
          const summary = await client.dataSummary(path)
          if (json) out(JSON.stringify(summary, null, 2))
          else out(renderDataSummary(summary))
          break
        }
        ...
        if (json) out(JSON.stringify(result, null, 2))
        else out(renderData(result.rows))
        break
      }
    }
    return exitCodes.success
