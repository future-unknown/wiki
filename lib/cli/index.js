...
import { renderTree, renderSearchResults, renderHistory, renderLog, renderRecords, renderDiff } from './render.js'
...
export async function main (argv) {
  ...
  try {
    switch (commandName) {
      ...
      case 'diff': {
        const [path] = positionals
        if (!path || positionals.length !== 1) throw new UsageError('usage: wiki diff <path>')
        if (values.revision && values.commit) throw new UsageError('--revision and --commit are exclusive')
        // A revision is addressed by id; a commit (or nothing: the
        // latest) names it through the page as it was there.
        let revisionId = values.revision
        if (!revisionId) {
          const node = await client.get(path, { commitId: parseIntOption(values.commit, 'commit') })
          revisionId = node.revisionId
        }
        const revision = await client.revision(path.split('.')[0], revisionId)
        if (json) out(JSON.stringify(revision, null, 2))
        else out(renderDiff(revision))
        break
      }
