...
/**
 * @param {Array<{ fullPath: string, count: number, latestTs: string }>} entries
 */
export function renderDataSummary (entries) {
  if (entries.length === 0) return 'no data'
  const width = Math.max(...entries.map((entry) => entry.fullPath.length))
  return entries
    .map((entry) => `${entry.fullPath.padEnd(width + 3)}${entry.count} observation${entry.count === 1 ? '' : 's'}, latest ${entry.latestTs}`)
    .join('\n')
}
