...
export function createWikiKit ({ db, records }) {
  ...
  const SUMMARY_BUMP_SQL = `
    INSERT INTO node_records (node_id, wiki_id, count, latest_ts, last_actor, updated_at)
    VALUES (?, ?, MAX(0, ?), ?, ?, ?)
  ...
  function bumpRecordSummary ({ wikiId, nodeId, delta, ts, actor }) {
    ...
    db.prepare(SUMMARY_BUMP_SQL).run(nodeId, wikiId, delta, ts ?? null, actorJson(actor), now(), delta)
  }
  ...
  function syncRecordSummary ({ wikiId, nodeId, items }) {
    ...
    db.prepare(SUMMARY_SYNC_SQL).run(nodeId, wikiId, items.length, latest, actorJson(actor), now())
  }
  ...
  return {
    ...
    async getWikiActivity ({ wikiId, except }) {
      ...
      const recordRows = db.prepare(
        'SELECT updated_at, last_actor FROM node_records WHERE wiki_id = ? ORDER BY updated_at DESC'
      ).all(wikiId).map((row) => ({ at: row.updated_at, actor: parseActor(row.last_actor) }))
