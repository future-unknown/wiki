...
export function createWikiKit ({ db, records }) {
  ...
  async function backfillRecordWriters () {
    const pending = db.prepare(
      'SELECT DISTINCT wiki_id FROM node_records WHERE written_at IS NULL OR last_actor IS NULL'
    ).all()
    for (const { wiki_id: wikiId } of pending) {
      ...
      for (const [nodeId, items] of byNode) syncRecordSummary({ wikiId, nodeId, items })
      // pages whose records are all gone: nothing to learn, stop asking
      db.prepare(`
        UPDATE node_records SET written_at = COALESCE(written_at, updated_at)
        WHERE wiki_id = ? AND written_at IS NULL
      `).run(wikiId)
    }
  }
  ...
  return {
    ...
    async getWikiActivity ({ wikiId, except }) {
      ...
      const latestOtherRecord = recordRows.find((entry) => !own(entry.actor)) ?? null
