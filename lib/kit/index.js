...
export function createWikiKit ({ db, records }) {
  ...
  const SUMMARY_BUMP_SQL = `
    INSERT INTO node_records (node_id, wiki_id, count, latest_ts, updated_at)
    VALUES (?, ?, MAX(0, ?), ?, ?)
  ...
  const SUMMARY_SYNC_SQL = `
    ...
    VALUES (?, ?, ?, ?, ?)
    ON CONFLICT(node_id) DO UPDATE SET
      count = excluded.count, latest_ts = excluded.latest_ts, updated_at = excluded.updated_at
  `

  function bumpRecordSummary ({ wikiId, nodeId, delta, ts }) {
    // The delta rides twice: clamped for a first row, raw for the update.
    db.prepare(SUMMARY_BUMP_SQL).run(nodeId, wikiId, delta, ts ?? null, now(), delta)
  }

  function syncRecordSummary ({ wikiId, nodeId, items }) {
    let latest = null
    for (const item of items) {
      if (typeof item._ts === 'string' && (latest === null || item._ts > latest)) latest = item._ts
    }
    db.prepare(SUMMARY_SYNC_SQL).run(nodeId, wikiId, items.length, latest, now())
  }
  ...
  async function drainLegacyObservations () {
    ...
    for (const row of rows) {
      ...
      try {
        ...
        bumpRecordSummary({ wikiId: row.wiki_id, nodeId: row.node_id, delta: 1, ts: row.ts })
      } catch (error) {
  ...
  return {
    ...
    async putRecord ({ wikiId, path, value, ts, ifVersion, actor }) {
      ...
      if (key) {
        ...
        if (ifVersion !== undefined) {
          ...
          bumpRecordSummary({ wikiId, nodeId: node.id, delta: 0, ts: timestamp })
        ...
        for (let attempt = 0; attempt < 3; attempt += 1) {
          ...
          try {
            ...
            bumpRecordSummary({ wikiId, nodeId: node.id, delta: current ? 0 : 1, ts: timestamp })
            return toRecord(item)
          } catch (error) {
            if (!error.conditionFailed) throw error
          }
      ...
      await records.put({ item, condition: 'attribute_not_exists(pk)' })
      bumpRecordSummary({ wikiId, nodeId: node.id, delta: 1, ts: tsIso })
      return toRecord(item)
    },
    ...
    async deleteRecord ({ wikiId, path, key, actor }) {
      ...
      bumpRecordSummary({ wikiId, nodeId: node.id, delta: -1 })
