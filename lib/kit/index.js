...
// Activity windows for getWikiActivity: a day, a week, and the week's
// day-by-day shape.
const ACTIVITY_DAY_MS = 24 * 60 * 60 * 1000
const ACTIVITY_WEEK_DAYS = 7
const ACTIVITY_WEEK_MS = ACTIVITY_WEEK_DAYS * ACTIVITY_DAY_MS
...
// What a revision did to its node, read against the previous revision:
// the first is a creation (a deleted node written again is one too), a
// tombstone a deletion, a new slug or parent a move, anything else an
// update.
function changeKind (row) {
  if (row.deleted === 1) return 'deleted'
  if (row.prev_slug === null || row.prev_deleted === 1) return 'created'
  if (row.prev_slug !== row.slug || row.prev_parent !== row.parent_id) return 'moved'
  return 'updated'
}

// A revision row with its commit: the shape node history and a single
// revision read share. The row's own columns come first; the commit's
// actor and message ride along so a revision says who made it.
const REVISION_SELECT = `
  SELECT r.id AS revision_id, r.node_id, r.commit_id, r.parent_id, r.slug, r.title, r.content,
...
export function createWikiKit ({ db, records }) {
  ...
  // The record summary projection: count and latest stamp per page.
  // `bump` moves it by a delta on a write; `sync` sets it from a full
  // read, which heals drift from retention expiry in the store. The
  // statements compile on first use: the table exists after migrate,
  // and the kit is constructed before that.
  // A bump is a write: it moves written_at and names the writer. A sync
  // is maintenance: it corrects count and latest stamp, and only fills
  // in a write time and writer the row never had (a store that predates
  // the projection), taking the latest record's own stamp for both.
  const SUMMARY_BUMP_SQL = `
    INSERT INTO node_records (node_id, wiki_id, count, latest_ts, last_actor, written_at, updated_at)
    VALUES (?, ?, MAX(0, ?), ?, ?, ?, ?)
    ON CONFLICT(node_id) DO UPDATE SET
      count = MAX(0, node_records.count + ?),
      latest_ts = CASE
        WHEN excluded.latest_ts IS NOT NULL AND
             (node_records.latest_ts IS NULL OR excluded.latest_ts > node_records.latest_ts)
        THEN excluded.latest_ts ELSE node_records.latest_ts END,
      last_actor = COALESCE(excluded.last_actor, node_records.last_actor),
      written_at = excluded.written_at,
      updated_at = excluded.updated_at
  `
  const SUMMARY_SYNC_SQL = `
    INSERT INTO node_records (node_id, wiki_id, count, latest_ts, last_actor, written_at, updated_at)
    VALUES (?, ?, ?, ?, ?, ?, ?)
    ON CONFLICT(node_id) DO UPDATE SET
      count = excluded.count, latest_ts = excluded.latest_ts,
      last_actor = COALESCE(node_records.last_actor, excluded.last_actor),
      written_at = COALESCE(node_records.written_at, excluded.written_at),
      updated_at = excluded.updated_at
  `

  function actorJson (actor) {
    return actor ? JSON.stringify({ type: actor.type, id: actor.id, onBehalfOf: actor.onBehalfOf ?? null }) : null
  }
  ...
  function bumpRecordSummary ({ wikiId, nodeId, delta, ts, actor }) {
    const at = now()
    // The delta rides twice: clamped for a first row, raw for the update.
    db.prepare(SUMMARY_BUMP_SQL).run(nodeId, wikiId, delta, ts ?? null, actorJson(actor), at, at, delta)
  }

  function syncRecordSummary ({ wikiId, nodeId, items }) {
    // Nothing to summarize makes no row: a page nobody has written to
    // has no record activity, and a read of it must not invent any.
    if (items.length === 0) {
      db.prepare('UPDATE node_records SET count = 0, latest_ts = NULL, updated_at = ? WHERE node_id = ?').run(now(), nodeId)
      return
    }
    let latest = null
    let actor = null
    for (const item of items) {
      if (typeof item._ts === 'string' && (latest === null || item._ts > latest)) {
        latest = item._ts
        actor = item._actor ?? null
      }
    }
    db.prepare(SUMMARY_SYNC_SQL).run(nodeId, wikiId, items.length, latest, actorJson(actor), latest, now())
  }

  function recordSummaries (wikiId) {
    const summaries = new Map()
    for (const row of db.prepare('SELECT node_id, count, latest_ts FROM node_records WHERE wiki_id = ?').all(wikiId)) {
      if (row.count > 0) summaries.set(row.node_id, { count: row.count, latestTs: row.latest_ts })
    }
    return summaries
  }
  ...
  const renameWikiTx = db.transaction(({ wikiId, slug }) => {
    const root = requireWiki(wikiId)
    if (root.slug === slug) return rowToNode(root)
    const taken = db.prepare(
      'SELECT id FROM nodes WHERE slug = ? AND parent_id IS NULL AND deleted = 0'
    ).get(slug)
    if (taken) throw new AlreadyExistsError('wiki already exists', { slug })
    db.prepare('UPDATE nodes SET slug = ? WHERE id = ?').run(slug, root.id)
    return rowToNode(requireWiki(wikiId))
  })
   ...
   */
  /**
   * One-time build of the record summary for an existing store: when
   * the projection is empty, every wiki's partition is read once and
   * each page's count and latest stamp written. Later writes and full
   * reads keep it moving; this only gives an upgrade its starting
   * point. Idempotent — a populated projection is left alone.
   */
  async function backfillRecordSummary () {
    const populated = db.prepare('SELECT 1 FROM node_records LIMIT 1').get()
    if (populated) return backfillRecordWriters()
    const wikis = db.prepare('SELECT id FROM nodes WHERE parent_id IS NULL AND deleted = 0').all()
    for (const { id: wikiId } of wikis) {
      const byNode = new Map()
      let cursor
      do {
        const { items, next } = await records.query({ pk: wikiId, from: '', to: '\uffff', limit: 1000, cursor })
        for (const item of items) {
          const nodeId = item.sk.slice(0, item.sk.indexOf('#'))
      ...
      for (const [nodeId, items] of byNode) syncRecordSummary({ wikiId, nodeId, items })
    }
  }

  /**
   * Rows from before the projection knew write times and writers get
   * both from the store: the latest record's stamp and actor. Runs
   * only while such rows exist, so it is a one-time cost per upgrade.
   */
  async function backfillRecordWriters () {
    // Rows a read of an empty page once created carry a read time as
    // their write time and no writer: they say nothing, so they stop
    // saying it.
    db.prepare('UPDATE node_records SET written_at = NULL WHERE last_actor IS NULL AND latest_ts IS NULL').run()
    const pending = db.prepare(
      'SELECT DISTINCT wiki_id FROM node_records WHERE (written_at IS NULL OR last_actor IS NULL) AND count > 0'
    ).all()
    for (const { wiki_id: wikiId } of pending) {
      const byNode = new Map()
      let cursor
      do {
        const { items, next } = await records.query({ pk: wikiId, from: '', to: '\uffff', limit: 1000, cursor })
  ...
  async function drainLegacyObservations () {
    ...
    for (const row of rows) {
      ...
      try {
        ...
        bumpRecordSummary({ wikiId: row.wiki_id, nodeId: row.node_id, delta: 1, ts: row.ts, actor: item._actor })
      } catch (error) {
  ...
  return {
    ...
    /**
     * Store a wiki under another slug. A slug is how a wiki is found,
     * not what it says, so this is no commit and leaves every revision
     * as written: history reads the same before and after. For hosts
     * moving their storage names; the slug must be free.
     */
    async renameWiki ({ wikiId, slug }) {
      assertValidSlug(slug)
      return renameWikiTx.immediate({ wikiId, slug })
    },
    ...
    /**
     * When the wiki last changed and by whom: its latest authored
     * commit and its latest record write, each as `{ at, actor }` or
     * null, plus `at` (the later of the two) and `othersAt` — the same
     * ignoring changes by `except`, an actor id whose own work, direct
     * or on their behalf, is not news to them. Reading surfaces turn
     * `othersAt` against a per-reader mark into "changed since you
     * last looked".
     */
    async getWikiActivity ({ wikiId, except }) {
      requireWiki(wikiId)
      const own = (actor) => except != null && (actor?.id === except || actor?.onBehalfOf === except)
      const commitToActivity = (row) => (row
        ? { at: row.created_at, actor: { type: row.actor_type, id: row.actor_id, onBehalfOf: row.on_behalf_of ?? null } }
        : null)
      const latestCommit = commitToActivity(db.prepare(
        'SELECT actor_type, actor_id, on_behalf_of, created_at FROM commits WHERE wiki_id = ? ORDER BY id DESC LIMIT 1'
      ).get(wikiId))
      const latestOtherCommit = except == null
        ? latestCommit
        : commitToActivity(db.prepare(`
            SELECT actor_type, actor_id, on_behalf_of, created_at FROM commits
            WHERE wiki_id = ? AND actor_id != ? AND (on_behalf_of IS NULL OR on_behalf_of != ?)
            ORDER BY id DESC LIMIT 1
          `).get(wikiId, except, except))
      const recordRows = db.prepare(`
        SELECT written_at, last_actor FROM node_records
        WHERE wiki_id = ? AND written_at IS NOT NULL ORDER BY written_at DESC
      `).all(wikiId).map((row) => ({ at: row.written_at, actor: parseActor(row.last_actor) }))
      const latestRecord = recordRows[0] ?? null
      // An unknown writer is no evidence that someone else wrote.
      const latestOtherRecord = recordRows.find((entry) => entry.actor && !own(entry.actor)) ?? null
      const later = (a, b) => (a && b ? (a > b ? a : b) : a ?? b ?? null)
      // How much, not only when: commits in the last day and week, the
      // week by day (oldest first, seven UTC days ending today), and how
      // many people they came from — an agent acting for someone is that
      // someone. Record writes keep no history, so they are not counted.
      const now = Date.now()
      const dayAgo = new Date(now - ACTIVITY_DAY_MS).toISOString()
      const weekAgo = new Date(now - ACTIVITY_WEEK_MS).toISOString()
      const counts = db.prepare(`
        SELECT COUNT(*) AS week,
               SUM(CASE WHEN created_at >= ? THEN 1 ELSE 0 END) AS day,
               COUNT(DISTINCT COALESCE(on_behalf_of, actor_id)) AS people
        FROM commits WHERE wiki_id = ? AND created_at >= ?
      `).get(dayAgo, wikiId, weekAgo)
      const byDay = new Map(db.prepare(`
        SELECT substr(created_at, 1, 10) AS day, COUNT(*) AS n
        FROM commits WHERE wiki_id = ? AND created_at >= ? GROUP BY day
      `).all(wikiId, weekAgo).map((row) => [row.day, row.n]))
      const days = Array.from({ length: ACTIVITY_WEEK_DAYS }, (_, index) => {
        const day = new Date(now - (ACTIVITY_WEEK_DAYS - 1 - index) * ACTIVITY_DAY_MS).toISOString().slice(0, 10)
        return byDay.get(day) ?? 0
      })
      return {
        authored: latestCommit,
        recorded: latestRecord,
        at: later(latestCommit?.at, latestRecord?.at),
        othersAt: later(latestOtherCommit?.at, latestOtherRecord?.at),
        commits: { day: counts.day ?? 0, week: counts.week ?? 0, days },
        people: { week: counts.people ?? 0 }
      }
    },

    /**
     * How many commits a wiki has taken since a moment, by others. `since`
     * is an ISO time (or absent: all of them); `except` sets one actor's
     * own work aside, agents acting for them included. The number behind
     * "changes since you last looked", counted in the store rather than
     * paged out of the log, so a busy agent day costs one query.
     */
    async countCommits ({ wikiId, since, except }) {
      requireWiki(wikiId)
      if (since !== undefined && since !== null && (typeof since !== 'string' || Number.isNaN(Date.parse(since)))) {
        throw new ValidationError('since must be an ISO timestamp', { since })
      }
      const row = db.prepare(`
        SELECT COUNT(*) AS n FROM commits
        WHERE wiki_id = ?
          AND (? IS NULL OR created_at > ?)
          AND (? IS NULL OR (actor_id != ? AND (on_behalf_of IS NULL OR on_behalf_of != ?)))
      `).get(wikiId, since ?? null, since ?? null, except ?? null, except ?? null, except ?? null)
      return row.n
    },
    ...
    async getTree ({ wikiId, path, depth, commitId, at }) {
      ...
      // A current tree says what each page's records amount to —
      // `records: { count, latestTs }`, or null — so reading surfaces
      // can mark the pages that carry data. History carries none: the
      // summary is a projection of now.
      const summaries = recordSummaries(wikiId)
      const nodes = getActiveSubtree(wikiId, relPath).map((row) => {
        const node = rowToNode(row)
        return { ...node, records: summaries.get(node.id) ?? null }
      })
    ...
    /**
     * One revision and the one before it on the same node: the pair a
     * diff reads. The revision carries its node's *current* path, like
     * a log change does, so a reader can authorize and link it. `kind`
     * says what the revision did (created, updated, moved, deleted);
     * `previous` is null for a node's first revision.
     */
    async getRevision ({ wikiId, revisionId }) {
      if (typeof revisionId !== 'string' || revisionId === '') {
        throw new ValidationError('revisionId must be a non-empty string', { revisionId })
      }
      requireWikiEverExisted(wikiId)
      const row = db.prepare(`
        ${REVISION_SELECT}
        WHERE r.wiki_id = ? AND r.id = ?
      `).get(wikiId, revisionId)
      if (!row) throw new NotFoundError('revision not found', { wikiId, revisionId })
      const before = db.prepare(`
        ${REVISION_SELECT}
        WHERE r.node_id = ? AND r.commit_id < ?
        ORDER BY r.commit_id DESC
        LIMIT 1
      `).get(row.node_id, row.commit_id)
      const revision = rowToRevision(row)
      revision.path = db.prepare('SELECT path FROM nodes WHERE id = ?').get(row.node_id).path
      revision.kind = changeKind({
        deleted: row.deleted,
        slug: row.slug,
        parent_id: row.parent_id,
        prev_slug: before ? before.slug : null,
        prev_parent: before ? before.parent_id : null,
        prev_deleted: before ? before.deleted : null
      })
      revision.previous = before ? rowToRevision(before) : null
      return revision
    },

    /**
     * The wiki's change log: commits newest first, each with the pages it
     * touched. A revision is read against the node's previous one, so
     * each change says what it was — created, updated, moved, deleted —
     * and names the node's *current* path (moves and deletes keep their
     * row), like notes do. `path` scopes the log to a subtree: commits
     * are listed when they touched it, with only the touched pages
     * inside it. `before` continues a listing from a commit id.
     */
    async getLog ({ wikiId, path, limit, before }) {
      const relPath = path ?? ''
      parseRelativePath(relPath)
      const max = limit === undefined ? 50 : limit
      if (!Number.isInteger(max) || max <= 0 || max > 500) {
      ...
      if (before !== undefined && (!Number.isInteger(before) || before <= 0)) {
        throw new ValidationError('before must be a positive commit id', { before })
      }
      requireWikiEverExisted(wikiId)
      const rows = db.prepare(`
        WITH scoped AS (
          SELECT r.id AS revision_id, r.node_id, r.commit_id, r.slug, r.title, r.deleted, r.parent_id,
                 n.path,
                 LAG(r.slug) OVER w AS prev_slug,
                 LAG(r.parent_id) OVER w AS prev_parent,
                 LAG(r.deleted) OVER w AS prev_deleted
          FROM node_revisions r
          JOIN nodes n ON n.id = r.node_id
          WHERE r.wiki_id = ?
          WINDOW w AS (PARTITION BY r.node_id ORDER BY r.commit_id)
        ),
        touched AS (
          SELECT DISTINCT c.id
          FROM commits c
          JOIN scoped s ON s.commit_id = c.id
          WHERE c.wiki_id = ?
            AND (? = '' OR s.path = ? OR s.path LIKE ? || '.%')
            AND (? IS NULL OR c.id < ?)
          ORDER BY c.id DESC
          LIMIT ?
        )
        SELECT c.id, c.actor_type, c.actor_id, c.on_behalf_of, c.message, c.created_at,
               s.revision_id, s.node_id, s.path, s.slug, s.title, s.deleted,
               s.prev_slug, s.prev_parent, s.prev_deleted, s.parent_id
        FROM touched t
        JOIN commits c ON c.id = t.id
        JOIN scoped s ON s.commit_id = c.id
        WHERE (? = '' OR s.path = ? OR s.path LIKE ? || '.%')
        ORDER BY c.id DESC, s.path
      `).all(
        wikiId, wikiId, relPath, relPath, relPath, before ?? null, before ?? null, max,
      ...
      for (const row of rows) {
        if (!current || current.id !== row.id) {
          current = {
            ...
            createdAt: row.created_at,
            message: row.message,
            actor: { type: row.actor_type, id: row.actor_id, onBehalfOf: row.on_behalf_of },
            changes: []
          }
    ...
    async putRecord ({ wikiId, path, value, ts, ifVersion, actor }) {
      ...
      if (key) {
        ...
        if (ifVersion !== undefined) {
          ...
          bumpRecordSummary({ wikiId, nodeId: node.id, delta: 0, ts: timestamp, actor: stamp })
        ...
        for (let attempt = 0; attempt < 3; attempt += 1) {
          ...
          try {
            ...
            bumpRecordSummary({ wikiId, nodeId: node.id, delta: current ? 0 : 1, ts: timestamp, actor: stamp })
            return toRecord(item)
          } catch (error) {
            if (!error.conditionFailed) throw error
          }
      ...
      await records.put({ item, condition: 'attribute_not_exists(pk)' })
      bumpRecordSummary({ wikiId, nodeId: node.id, delta: 1, ts: tsIso, actor: stamp })
      return toRecord(item)
    },

    /**
     * Records of the page at `path`. With `key`, exactly that record
     * (combines with nothing else). Otherwise a page of records in
     * sort order — time on unkeyed pages, key on keyed ones — or in
     * reverse with `reverse` (newest first on a log), bounded by
     * `since`/`until` (ISO-8601), capped by `limit`, and continued
     * with `cursor` when a previous page reported one. `latest`
     * returns only the newest record. Returns `{ record }` for a key
     * read, `{ records, cursor? }` otherwise.
     */
    async getRecords ({ wikiId, path, key, latest, since, until, limit, cursor, reverse }) {
      requireRecords()
      const relPath = path ?? ''
      parseRelativePath(relPath)
      requireWiki(wikiId)
      ...
      if (key !== undefined && key !== null) {
        ...
        if (latest || reverse || since !== undefined || until !== undefined ||
      ...
      if (latest) {
        if (reverse || since !== undefined || until !== undefined || limit !== undefined || cursor !== undefined) {
          throw new ValidationError('latest cannot be combined with reverse, since, until, limit, or cursor', {})
        }
      ...
      if (reverse !== undefined && typeof reverse !== 'boolean') {
        throw new ValidationError('reverse must be a boolean', { reverse })
      }
      ...
      // One item past the limit says whether a page follows. The
      // store's own continuation key is no evidence: DynamoDB returns
      // one whenever the limit is met, the last item included.
      const { items } = await records.query({
        pk: wikiId,
      ...
      // A read that saw the whole set is the truth about it: it heals
      // the summary after retention expired records the kit never saw.
      const whole = !more && sinceIso === undefined && untilIso === undefined && cursor === undefined
      if (whole) syncRecordSummary({ wikiId, nodeId: node.id, items: page })
      return {
        records: page.map(toRecord),
        cursor: more ? encodeCursor({ pk: last.pk, sk: last.sk }) : undefined
      }
    },
    ...
    async deleteRecord ({ wikiId, path, key, actor }) {
      ...
      bumpRecordSummary({ wikiId, nodeId: node.id, delta: -1, actor: { type: actor.type, id: actor.id, onBehalfOf: actor.onBehalfOf ?? null } })
