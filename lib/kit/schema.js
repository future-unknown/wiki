...
export const migrations = [
  ...
  {
    id: '005-record-summary-actor',
    sql: `
      -- Who last wrote to the page's record set (a JSON actor stamp),
      -- so a wiki's recent activity can say whose it was.
      ALTER TABLE node_records ADD COLUMN last_actor TEXT;
    `
  },
  {
    id: '006-record-summary-written',
    sql: `
      -- When the page's record set was last written to. updated_at moves
      -- on every maintenance of the projection (a full read re-syncs
      -- it); activity must not.
      ALTER TABLE node_records ADD COLUMN written_at TEXT;
    `
  }
]

/**
 * Apply pending migrations. Idempotent, and safe when several
 * connections — other processes included — migrate one file at once:
 * the pending set is read and applied under the write lock, taken up
 * front (BEGIN IMMEDIATE), so whoever waits for it finds the work done.
