...
export class WikiClient {
  ...
  /**
   * Append an observation (any JSON value) to a page's data channel.
   *
   * @param {string} path
   * @param {unknown} payload
   * @param {{ ts?: string }} [options] backfill an observation time (ISO-8601)
   */
  async push (path, payload, { ts } = {}) {
    return this.rpc('wiki.push', { path, payload, ts })
  }

  /**
   * Read a page's data channel, ascending by observation time.
   * `latest` returns only the newest observation and cannot combine
   * with the range options.
   *
   * @param {string} path
   * @param {{ latest?: boolean, since?: string, until?: string, limit?: number }} [options]
   * @returns {Promise<{ fullPath: string, rows: Array<object> }>}
   */
  async data (path, { latest, since, until, limit } = {}) {
    return this.rpc('wiki.data', { path, latest, since, until, limit })
  }

  /**
   * Which pages in the subtree carry observations: one entry per page
   * with a non-empty data channel — count and latest observation time.
   *
   * @param {string} path subtree root (the wiki slug for the whole wiki)
   * @returns {Promise<Array<{ fullPath: string, count: number, latestTs: string }>>}
   */
  async dataSummary (path) {
    return this.rpc('wiki.dataSummary', { path })
  }
