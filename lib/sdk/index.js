...
export class WikiClient {
   ...
   * records ascending by sort order, bounded by `since`/`until`,
   * capped by `limit`, continued with `cursor`; `latest` returns only
   * the newest: `{ fullPath, records, cursor? }`.
   *
   * @param {string} path
   * @param {{ key?: string, latest?: boolean, since?: string,
   *           until?: string, limit?: number, cursor?: string }} [options]
   */
  async data (path, { key, latest, since, until, limit, cursor } = {}) {
    return this.rpc('wiki.data', { path, key, latest, since, until, limit, cursor })
  }
